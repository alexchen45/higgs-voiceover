...
local function quick_place(takes, mode)
  ...
  quick_note((placed == 1 and ("Placed on “%s” %s."):format(app.cfg.vo_track_name, where)
                          or ("Placed %d clips on “%s”, starting %s."):format(placed, app.cfg.vo_track_name, where)), "ok")
  return true
end
...
local function finish_take(path)
  ...
  if app.cfg.pause_enabled ~= false and tostring(path):lower():match("%.wav$") then
    ...
    if ms > 0 and not U.wav_append_silence(path, ms / 1000) then Log.warn("could not append silence to " .. path) end
  end
    ...
    ui:Stack{
        ...
        ui:VGroup{ ID = "PageSetup",
          ...
          ui:HGroup{
            ...
            ui:VGroup{
              ...
              ui:VGap(16),
              -- The analytics question, asked once and up front, with what is
              -- never collected right under it. Same words as Settings.
              ui:CheckBox{ ID = "SetupAnalyticsChk", Text = "Share anonymous usage data to help improve Higgs VoiceOver",
                           Weight = 0, StyleSheet = T.checkbox() },
              ui:VGap(2),
              ui:HGroup{
                Weight = 0, Spacing = 0,
                -- 22 px: the checkbox's indicator (14) and its spacing (8), so
                -- the note starts under the label, not under the box.
                ui:Label{ Weight = 0, MinimumSize = { 22, 0 }, MaximumSize = { 22, 4000 } },
                ui:Label{ Text = "Your scripts, voices and generated audio are never shared.", Weight = 1,
        ...
        ui:VGroup{ ID = "PageQuick",
          ...
          ui:HGroup{
            ...
            ui:VGroup{
              ...
              -- What happens when a run finishes, then the run itself.
              ui:HGroup{
                Weight = 0, Spacing = S.gap,
                ui:CheckBox{ ID = "AutoPreviewChk", Text = "Play when done", Weight = 0, StyleSheet = T.checkbox(),
                             ToolTip = "Start the audio preview as soon as generation finishes." },
                ui:Label{ Weight = 0, MinimumSize = { S.group - S.gap, 0 } },
                ui:CheckBox{ ID = "AutoPlaceChk", Text = "Place on timeline when done", Weight = 0, StyleSheet = T.checkbox(),
              ...
              card({
                ...
                -- Placing: the clip on show, or the whole run.
                ui:HGroup{
                  Weight = 0, Spacing = S.gap,
                  ui:Label{ Weight = 1 },
      ...
      ui:VGroup{ ID = "TabSettings",
        ...
        ui:VGroup{
          ...
          -- Records the answer only: nothing is collected or sent, and the
          -- tooltip says so rather than implying a pipeline that does not exist.
          settings_row("", ui:CheckBox{ ID = "AnalyticsChk", Text = "Share anonymous usage data to help improve Higgs VoiceOver", Weight = 1,
                                        StyleSheet = T.checkbox(),
                                        ToolTip = "Nothing is collected today. If this is ever switched on it will be counts of generations and errors — never your text, your audio or your key." }),
          settings_row("Logs",
        ...
        ui:Label{ Weight = 1 },
        -- Settings save as they change, like Resolve's own preferences.
        ui:HGroup{
          Weight = 0, Spacing = S.gap,
...
local UPDATE_REPO = "alexchen45/higgs-voiceover"
...
local function push_settings_to_ui()
  ...
  itm.AnalyticsChk.Checked = app.cfg.analytics and true or false
  itm.SetupAnalyticsChk.Checked = app.cfg.analytics and true or false
  refresh_update_ui()
...
local function test_key(key, status_label, on_ok, context)
  ...
  if context ~= "setup" then Config.save(app.cfg) end
...
local function wire_setup()
  ...
  win.On.SetupAnalyticsChk.Clicked = function()
    app.cfg.analytics = itm.SetupAnalyticsChk.Checked and true or false
    Config.save(app.cfg)
    suppress = true
    itm.AnalyticsChk.Checked = app.cfg.analytics
    suppress = false
    Log.info("analytics " .. (app.cfg.analytics and "on" or "off"), { where = "setup" })
  end
...
local function wire_settings()
  ...
  -- Every field saves as it changes; the footer confirms it.
  local function save_settings()
    if suppress then return end
    local before = U.json.encode({ Config.get_api_key(app.cfg), app.cfg.output_dir, app.cfg.vo_track_name,
                                   app.cfg.output_format, app.cfg.replace_mode, app.cfg.clip_name, app.cfg.check_updates,
                                   app.cfg.pause_enabled, app.cfg.pause_ms, app.cfg.analytics })
    Config.set_api_key(app.cfg, U.trim(itm.ApiKeyEdit.Text))
    app.cfg.output_dir = U.trim(itm.OutDir.Text) ~= "" and itm.OutDir.Text or Config.default_output_dir()
    app.cfg.vo_track_name = U.trim(itm.TrackName.Text) ~= "" and itm.TrackName.Text or Config.DEFAULTS.vo_track_name
    app.cfg.output_format = itm.FormatCombo.CurrentText
    app.cfg.pause_enabled = itm.PauseChk.Checked and true or false
    app.cfg.pause_ms = math.max(0, math.min(5000, tonumber(itm.PauseMs.Text) or 400))
    app.cfg.check_updates = itm.AutoUpdateChk.Checked and true or false
    app.cfg.analytics = itm.AnalyticsChk.Checked and true or false
    app.cfg.clip_name = { words = itm.NameWordsChk.Checked and true or false,
                          date = itm.NameDateChk.Checked and true or false,
                          time = itm.NameTimeChk.Checked and true or false,
                          take = false }
    refresh_name_preview()
    refresh_out_dir()
    local after = U.json.encode({ Config.get_api_key(app.cfg), app.cfg.output_dir, app.cfg.vo_track_name,
                                  app.cfg.output_format, app.cfg.replace_mode, app.cfg.clip_name, app.cfg.check_updates,
                                  app.cfg.pause_enabled, app.cfg.pause_ms, app.cfg.analytics })
    if after == before then return end   -- widgets settling at startup, not a change
    local names = app.cfg.clip_name or {}
    Log.info("settings changed", { format = app.cfg.output_format, folder = Log.q(Log.path_safe(app.cfg.output_dir)),
      track = Log.q(app.cfg.vo_track_name),
      names = (names.words ~= false and "words" or "") .. (names.date and "+date" or "") .. (names.time and "+time" or "") .. (names.take and "+take" or ""),
      updates = tostring(app.cfg.check_updates), analytics = tostring(app.cfg.analytics),
      pause_ms = app.cfg.pause_enabled and app.cfg.pause_ms or 0 })
    Config.save(app.cfg)
    itm.SettingsStatus.StyleSheet = T.label("meta")
    itm.SettingsStatus.Text = "Saved"
    saved_at = os.time()
  end
  -- Typing a key saves it but only Test connection vouches for it; the
  -- Generate page changes only when a key appears or disappears.
  win.On.ApiKeyEdit.TextChanged = function()
    if suppress then return end
    if U.trim(itm.ApiKeyEdit.Text) == Config.get_api_key(app.cfg) then return end   -- widget settling, not typing
    local had = Config.get_api_key(app.cfg) ~= ""
    save_settings()
    local has = Config.get_api_key(app.cfg) ~= ""
    if has then
      itm.SettingsKeyStatus.StyleSheet = T.label("meta")
      itm.SettingsKeyStatus.Text = "Key saved — test the connection to confirm it works."
    end
    if had ~= has then connected = false; route() end
  end
  win.On.OutDir.TextChanged = save_settings
  win.On.TrackName.TextChanged = save_settings
  win.On.PauseChk.Clicked = save_settings
  win.On.PauseMs.TextChanged = save_settings
  win.On.FormatCombo.CurrentIndexChanged = save_settings
  win.On.AutoUpdateChk.Clicked = save_settings
  win.On.AnalyticsChk.Clicked = save_settings
  ...
  win.On.NameWordsChk.Clicked = save_settings
  win.On.NameDateChk.Clicked = save_settings
  win.On.NameTimeChk.Clicked = save_settings
...
function M.run(context)
  ...
  pcall(function() win:RecalcLayout() end)
  M.debug = { route = route, show_tab = show_tab, app = app }   -- harness hook
