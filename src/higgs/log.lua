...
-- metric (a measured outcome, the same numbers analytics will carry).
--
...
--- Counters for the session summary and the analytics bundle.
-- counts[name] = number of times; sums[name.field] = running total.
M.counts, M.sums = {}, {}
M.session = nil       -- random id per launch, so analytics can group lines
...
--- What analytics will attach when it is switched on: this session's
-- totals and its log, redacted. Nothing sends it yet — see TODO.
function M.bundle()
